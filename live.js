...
async function patientForm() {
  ...
  const provinceLabel=document.createElement("label"),cityLabel=document.createElement("label");
  provinceLabel.innerHTML='State / Province<select name="province_id" id="patient-province"><option value="">Select country first</option></select>';
  cityLabel.innerHTML='City<select name="city_id" id="patient-city"><option value="">Select state / province first</option></select>';
  addressLine.before(provinceLabel,cityLabel);
  const loadCities=async()=>{const countryId=modal.querySelector("#country-select").value,provinceId=modal.querySelector("#patient-province").value,city=modal.querySelector("#patient-city");if(!countryId){city.innerHTML='<option value="">Select country first</option>';return;}let query=supabase.from("cities").select("id,default_name").eq("country_id",Number(countryId)).order("default_name");if(provinceId)query=query.eq("province_id",Number(provinceId));const {data,error}=await query;if(error){notify(error.message);return;}city.innerHTML=optionList(data||[],item=>item.default_name,"Select city");};
  const loadProvinces=async()=>{const countryId=modal.querySelector("#country-select").value,province=modal.querySelector("#patient-province");if(!countryId){province.innerHTML='<option value="">Select country first</option>';await loadCities();return;}const {data,error}=await supabase.from("provinces").select("id,default_name").eq("country_id",Number(countryId)).order("default_name");if(error){notify(error.message);return;}province.innerHTML=optionList(data||[],item=>item.default_name,"Select state / province");await loadCities();};
  modal.querySelector("#patient-province").addEventListener("change",loadCities);
  ...
  modal.querySelector("#patient-form").addEventListener("submit", async event => {
    ...
    const { data: address, error: addressError } = await supabase.from("addresses").insert({ line1: values.line1, country_id: Number(values.country_id),province_id:values.province_id?Number(values.province_id):null,city_id:values.city_id?Number(values.city_id):null,postal_code_text:values.postal_code_text||null }).select("id").single();
