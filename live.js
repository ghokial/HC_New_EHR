...
const ACTIVE_FACILITY_KEY="healthcarology_active_facility";
...
async function verifiedOnline(){if(!navigator.onLine)return false;try{const response=await fetch(`${SUPABASE_URL}/auth/v1/health`,{cache:"no-store",headers:{apikey:SUPABASE_PUBLISHABLE_KEY}});return response.ok}catch{return false}}

async function loadClinicalContext() {
  if (!session) return false;
  const [{data:provider},{data:memberships},{data:role}] = await Promise.all([
    supabase.from("providers").select("id").eq("user_id",session.user.id).maybeSingle(),
    supabase.from("facility_memberships").select("facility_id,facilities(id,name,slug,facility_number,facility_type)").eq("user_id",session.user.id).eq("active",true),
    supabase.from("user_roles").select("roles(role_code)").eq("user_id",session.user.id).limit(1).maybeSingle()
  ]);
  currentProviderId=provider?.id||null;
  let choices=(memberships||[]).map(item=>item.facilities).filter(Boolean),roleCode=role?.roles?.role_code||"";
  if(["root","platform_agent"].includes(roleCode)){const {data:allFacilities}=await supabase.from("facilities").select("id,name,slug,facility_number,facility_type").eq("active",true).order("name");choices=allFacilities||choices}
  const stored=sessionStorage.getItem(ACTIVE_FACILITY_KEY),selected=choices.find(item=>item.id===stored);
  if(!selected&&choices.length>1){facilityChoiceGate(choices);return false}
  const facility=selected||choices[0];currentFacilityId=facility?.id||null;
  if(currentFacilityId){sessionStorage.setItem(ACTIVE_FACILITY_KEY,currentFacilityId);const {error}=await supabase.from("active_facility_context").upsert({user_id:session.user.id,facility_id:currentFacilityId,selected_at:new Date().toISOString()},{onConflict:"user_id"});if(error){notify(error.message);return false}}
  if(!currentFacilityId){notify("No authorized facility is selected. Use Switch to choose a facility before opening encounters.");return false}
  return true;
}

function facilityChoiceGate(facilities){const gate=document.createElement("div");gate.className="auth-gate";gate.innerHTML=`<section class="auth-card auth-card-wide"><img class="portal-logo" src="/assets/images/healthcarology-logo.png" alt="Healthcarology"><p class="eyebrow">Facility selection</p><h1>Choose your workspace</h1><p>Your credentials work across your authorized facilities. Patient and clinical data remain separated by the selected facility.</p><div class="facility-choices">${facilities.map(f=>`<button type="button" class="mfa-method" data-facility-choice="${safe(f.id)}"><span><strong>${safe(f.name)}</strong><small>${safe(f.facility_number||f.facility_type||f.slug)}</small></span></button>`).join("")}</div></section>`;document.body.append(gate);gate.querySelectorAll("[data-facility-choice]").forEach(button=>button.addEventListener("click",()=>{sessionStorage.setItem(ACTIVE_FACILITY_KEY,button.dataset.facilityChoice);location.reload()}))}

async function clinicalChoices() {
  if(!currentFacilityId) throw new Error("No facility is selected. Use Switch to choose an authorized facility before opening an encounter.");
  const [{data:patients,error:patientError},{data:services,error:serviceError},{data:medications,error:medicationError}] = await Promise.all([
    supabase.from("patients").select("id,first_name,last_name,snau").eq("facility_id",currentFacilityId).order("last_name"),
...
function authGate() {
  ...
  const language=gate.querySelector("#login-language");language.value=localStorage.getItem("hc_locale")||"en";language.addEventListener("change",()=>{localStorage.setItem("hc_locale",language.value);document.documentElement.lang=language.value;applyLocale(gate,language.value)});applyLocale(gate,language.value);
...
async function patientForm() {
  const membership={facility_id:currentFacilityId};
  ...
  const callingCodes=await fetch("/assets/country-calling-codes.json").then(response=>response.json()),callingInput=modal.querySelector('[name="country_calling_code"]'),callingSelect=document.createElement("select");callingSelect.name="country_calling_code";callingSelect.id="patient-calling-code";callingSelect.setAttribute("aria-label","Country calling code");callingSelect.innerHTML=callingCodes.map(item=>`<option value="${safe(item.calling_code)}" data-iso2="${safe(item.iso2)}">${safe(item.name)} (${safe(item.calling_code)})</option>`).join("");callingInput.replaceWith(callingSelect);
  ...
  modal.querySelector("#country-select").addEventListener("change",async event=>{const iso2=event.target.selectedOptions[0]?.dataset.iso2,match=callingCodes.find(item=>item.iso2===iso2);if(match)callingSelect.value=match.calling_code;await loadProvinces();await refreshAffiliations();});
  ...
  const patientIntakeForm=modal.querySelector("#patient-form"),{data:savedDraft}=await supabase.from("patient_intake_drafts").select("draft_data").eq("user_id",session.user.id).eq("facility_id",currentFacilityId).maybeSingle();if(savedDraft?.draft_data)for(const [name,value] of Object.entries(savedDraft.draft_data)){const field=patientIntakeForm.elements.namedItem(name);if(field&&typeof value==="string")field.value=value}let draftTimer;patientIntakeForm.addEventListener("input",()=>{clearTimeout(draftTimer);draftTimer=setTimeout(async()=>{const draft_data=Object.fromEntries(new FormData(patientIntakeForm));await supabase.from("patient_intake_drafts").upsert({user_id:session.user.id,facility_id:currentFacilityId,draft_data,updated_at:new Date().toISOString()},{onConflict:"user_id,facility_id"})},500)});
  patientIntakeForm.addEventListener("submit", async event => {
    ...
    await supabase.from("patient_intake_drafts").delete().eq("user_id",session.user.id).eq("facility_id",currentFacilityId);closeModal(); notify(created.snau?`Patient created. Permanent UNIN: ${created.snau}`:"Patient created. UNIN is pending an approved country registry rule."); await hydrateLivePatients();
  });
}
...
async function userForm() {
  ...
  const displayLabel=modal.querySelector('[name="display_name"]').closest("label");displayLabel.innerHTML='Name<div class="name-grid"><input name="first_name" placeholder="First name" required><input name="middle_name" placeholder="Middle name"><input name="last_name" placeholder="Last name" required></div>';const phoneInput=modal.querySelector('[name="phone"]'),phoneCodes=await fetch("/assets/country-calling-codes.json").then(r=>r.json()),phoneCode=document.createElement("select");phoneCode.name="phone_calling_code";phoneCode.setAttribute("aria-label","Country calling code");phoneCode.innerHTML=phoneCodes.map(x=>`<option value="${safe(x.calling_code)}">${safe(x.name)} (${safe(x.calling_code)})</option>`).join("");phoneInput.pattern="[0-9 ()-]{6,18}";phoneInput.placeholder="Phone number";const phoneGrid=document.createElement("div");phoneGrid.className="phone-grid phone-country-grid";phoneInput.before(phoneGrid);phoneGrid.append(phoneCode,phoneInput);
  modal.querySelector("[data-cancel]").addEventListener("click",closeModal);
  modal.querySelector("#user-form").addEventListener("submit", async event => {
    event.preventDefault();
    const body = Object.fromEntries(new FormData(event.target));body.display_name=[body.first_name,body.middle_name,body.last_name].filter(Boolean).join(" ");body.phone=`${body.phone_calling_code}${String(body.phone).replace(/\D/g,"")}`;
...
async function hydrateLivePatients() {
  ...
  const { data, error } = await supabase.from("patients").select("id,first_name,middle_name,last_name,mrn,date_of_birth,sex,snau,life_status").eq("facility_id",currentFacilityId).order("last_name");
...
function patientActionMenu(patientId,patientName){
  showModal("Patient options",`<div class="live-banner"><strong>${safe(patientName)}</strong></div><div class="patient-actions"><button type="button" class="button primary" data-patient-encounter="${patientId}">Start new encounter</button><button type="button" class="button secondary" data-patient-profile="${patientId}">View full patient profile</button><button type="button" class="button secondary" data-patient-vitals="${patientId}">Vital-sign graphs</button><button type="button" class="button secondary" data-patient-deleted="${patientId}">Deleted record history</button></div>`);
}

async function openDeletedRecordHistory(patientId){const {data,error}=await supabase.from("deleted_record_archive").select("id,source_table,record_data,deleted_by_name,deleted_at").contains("record_data",{patient_id:patientId}).order("deleted_at",{ascending:false});if(error)return notify(error.message);showModal("Deleted record history",`<div class="live-banner">Deleted clinical information is retained, crossed out, and attributed to the person who deleted it.</div><div class="deleted-history">${(data||[]).map(item=>`<article><del>${safe(item.source_table)} · ${safe(JSON.stringify(item.record_data))}</del><small>Deleted by ${safe(item.deleted_by_name)} · ${safe(new Date(item.deleted_at).toLocaleString())}</small></article>`).join("")||'<p class="empty">No deleted records for this patient.</p>'}</div>`)}

const vitalDefinitions={weight_kg:["Weight","kg"],height_cm:["Height","cm"],bmi:["BMI",""],temperature_c:["Temperature","°C"],oxygen_saturation:["Oxygen saturation","%"],respiratory_rate:["Respiratory rate","/min"],heart_rate:["Heart rate","/min"],systolic_bp:["Systolic pressure","mmHg"],diastolic_bp:["Diastolic pressure","mmHg"],blood_glucose:["Blood glucose",""],pain_score:["Pain score","/10"],head_circumference_cm:["Head circumference","cm"],mid_upper_arm_circumference_cm:["Mid-upper arm circumference","cm"]};
async function openVitalTrends(patientId){
  try{const [vitalsResult,rulesResult]=await Promise.all([supabase.from("vital_sign_observations").select(`recorded_at,${Object.keys(vitalDefinitions).join(",")}`).eq("patient_id",patientId).order("recorded_at"),currentFacilityId?supabase.from("vital_sign_alert_rules").select("vital_code,display_name,unit,normal_low,normal_high,critical_low,critical_high").eq("facility_id",currentFacilityId).eq("active",true):Promise.resolve({data:[],error:null})]);if(vitalsResult.error||rulesResult.error)throw(vitalsResult.error||rulesResult.error);const series=Object.entries(vitalDefinitions).map(([code,[label,unit]])=>({code,label,unit,rule:(rulesResult.data||[]).find(rule=>rule.vital_code===code),points:(vitalsResult.data||[]).filter(row=>row[code]!==null).map(row=>({at:row.recorded_at,value:Number(row[code])}))})).filter(item=>item.points.length);showModal("Vital-sign trends",`<label>View period<select data-vital-period><option value="7">7 days</option><option value="30" selected>30 days</option><option value="90">90 days</option><option value="365">1 year</option><option value="all">All available</option></select></label><div class="vital-trends">${series.length?series.map(item=>`<article class="vital-trend" data-vital-series='${safe(JSON.stringify(item))}'><header><strong>${safe(item.label)}</strong><span data-vital-status></span></header><div class="vital-latest"></div><svg viewBox="0 0 300 90" role="img" aria-label="${safe(item.label)} trend"><polyline></polyline></svg><small>${item.rule?`Facility-approved normal range: ${safe(item.rule.normal_low??"—")}–${safe(item.rule.normal_high??"—")} ${safe(item.rule.unit)}`:"No facility-approved threshold configured"}</small></article>`).join(""):'<p class="empty">No vital signs have been entered.</p>'}</div>`);const period=modal.querySelector("[data-vital-period]");const draw=()=>modal.querySelectorAll("[data-vital-series]").forEach(card=>{const item=JSON.parse(card.dataset.vitalSeries),cutoff=period.value==="all"?0:Date.now()-Number(period.value)*86400000,points=item.points.filter(point=>new Date(point.at).getTime()>=cutoff),latest=points.at(-1),badge=card.querySelector("[data-vital-status]");if(!latest){card.querySelector("polyline").setAttribute("points","");card.querySelector(".vital-latest").textContent="No readings in this period";badge.textContent="";return}const values=points.map(point=>point.value),min=Math.min(...values),span=Math.max(...values)-min||1;card.querySelector("polyline").setAttribute("points",points.map((point,index)=>`${points.length===1?150:index*300/(points.length-1)},${82-(point.value-min)*70/span}`).join(" "));card.querySelector(".vital-latest").textContent=`${latest.value} ${item.unit} · ${new Date(latest.at).toLocaleString()}`;const r=item.rule;let state="unconfigured";if(r)state=(r.critical_low!==null&&latest.value<Number(r.critical_low))||(r.critical_high!==null&&latest.value>Number(r.critical_high))?"dangerously-abnormal":(r.normal_low!==null&&latest.value<Number(r.normal_low))||(r.normal_high!==null&&latest.value>Number(r.normal_high))?"out-of-range":"normal";badge.className=`result-interpretation ${state}`;badge.textContent=interpretationLabel(state)});period.addEventListener("change",draw);draw()}catch(error){notify(error.message)}}
...
async function patientProfile(patientId,inline=false){
  try{
    ...
    const results=await Promise.all([
      ...
      supabase.from("deleted_record_archive").select("id,source_table,record_data,deleted_by_name,deleted_at").contains("record_data",{patient_id:patientId}).order("deleted_at",{ascending:false})
    ]);
    const failed=results.find(result=>result.error);if(failed)throw failed.error;
    const [contacts,addressResult,encounters,diagnoses,orders,medications,notes,deletedRecords]=results.map(result=>result.data);
...
async function hydrateClinicalView(viewName) {
  ...
  if(viewName==="encounters"){
    ({data,error}=await supabase.from("encounters").select("id,encounter_type,start_at,status,patients!inner(first_name,last_name,facility_id),services(name)").eq("patients.facility_id",currentFacilityId).order("start_at",{ascending:false}).limit(100));
    if(error){table.innerHTML=`<tr><td colspan="3" class="empty">${safe(error.message)}</td></tr>`;return}table.innerHTML=(data||[]).length?(data||[]).map(item=>`<tr><td><button type="button" class="patient-name-link" data-encounter-workflow="${item.id}">${safe(item.encounter_type)}</button></td><td>${safe(`${item.patients?.first_name||""} ${item.patients?.last_name||""}`.trim())} · ${safe(new Date(item.start_at).toLocaleString())}</td><td><span class="status ${safe(item.status)}">${safe(item.status)}</span></td></tr>`).join(""):`<tr><td colspan="3" class="empty">No live records yet.</td></tr>`;table.querySelectorAll("[data-encounter-workflow]").forEach(button=>button.addEventListener("click",()=>openEncounterWorkflow(Number(button.dataset.encounterWorkflow))));return;
  } else if(viewName==="orders"){
    ({data,error}=await supabase.from("orders").select("id,order_type,status,created_at,patients!inner(first_name,last_name,facility_id),services(name)").eq("patients.facility_id",currentFacilityId).order("created_at",{ascending:false}).limit(100));
    ...
    ({data,error}=await supabase.from("medication_orders").select("id,dose,route,frequency,status,patients!inner(first_name,last_name,facility_id),medications(name,strength)").eq("patients.facility_id",currentFacilityId).order("id",{ascending:false}).limit(100));
    ...
    ({data,error}=await supabase.from("clinical_notes").select("id,note_type,status,created_at,patients!inner(first_name,last_name,facility_id)").eq("patients.facility_id",currentFacilityId).order("created_at",{ascending:false}).limit(100));
...
async function hydrateDashboard(){
  ...
  const [{count:encounters},{count:orders},{count:results},{count:patients},{data:schedule}] = await Promise.all([
    supabase.from("encounters").select("id,patients!inner(facility_id)",{count:"exact",head:true}).eq("patients.facility_id",currentFacilityId).gte("start_at",dayStart.toISOString()),
    supabase.from("orders").select("id,patients!inner(facility_id)",{count:"exact",head:true}).eq("patients.facility_id",currentFacilityId).in("status",["pending","in_progress"]),
    ...
    supabase.from("patients").select("id",{count:"exact",head:true}).eq("facility_id",currentFacilityId),
    supabase.from("encounters").select("id,start_at,status,encounter_type,patients!inner(first_name,last_name,date_of_birth,facility_id),services(name)").eq("patients.facility_id",currentFacilityId).gte("start_at",dayStart.toISOString()).order("start_at").limit(20)
  ]);
...
async function hydrateTriage(){
  ...
  const {data,error}=await supabase.from("care_queue_entries").select("id,reference_number,started_at,priority,patient_id,department_id,assigned_provider_id,patients(first_name,last_name,snau),departments(name)").eq("facility_id",currentFacilityId).eq("queue_stage","triage").order("priority",{ascending:false}).order("started_at");
...
const workflowSteps=[
  ["subjective","Subjective","Record the patient’s reported symptoms, concerns, and history."],
  ["objective","Objective","Record observed and measured clinical facts."],
  ["assessment","Assessment","Record the clinician’s assessment."],
  ["plan","Plan","Record the care plan."],
  ["physical_exam","Physical examination","Record the examination performed and findings."],
  ["presumed_diagnosis","Presumed diagnosis","Enter the working diagnosis in free text."],
  ["laboratory","Laboratory examinations","Select one or more laboratory examinations."],
  ["imaging","Imaging examinations","Select one or more imaging examinations."],
  ["diagnosis","Diagnosis","Review decision support and select the certified ICD-11 diagnosis."],
  ["prescription","Prescription","Record the prescription issued for this encounter."],
  ["treatment_plan","Treatment plan","Record treatment instructions and the ongoing care plan."],
  ["follow_up","Follow-up appointment","Schedule or document the required follow-up."],
  ["conclusion","Encounter conclusion","Record the patient status and any referral, admission, or transfer action."]
];

async function openEncounterWorkflow(encounterId){
  try{
    const [encounterResult,workflowResult,catalogResult,serviceResult]=await Promise.all([
      supabase.from("encounters").select("id,patient_id,status,patients(first_name,last_name,snau)").eq("id",encounterId).single(),
      supabase.from("encounter_clinical_workflows").select("*").eq("encounter_id",encounterId).maybeSingle(),
      supabase.from("clinical_order_catalog").select("id,order_type,code,name,use_text").in("order_type",["lab","imaging"]).eq("active",true).order("name"),
      supabase.from("services").select("id,name,departments(name)").order("name")
    ]);const error=encounterResult.error||workflowResult.error||catalogResult.error||serviceResult.error;if(error)throw error;
    const encounter=encounterResult.data,catalog=catalogResult.data||[],services=serviceResult.data||[];
    let workflow=workflowResult.data||{encounter_id:encounterId,patient_id:encounter.patient_id,current_step:"subjective"};
    showModal("Clinical encounter",`<div class="live-banner"><strong>${safe(`${encounter.patients?.first_name||""} ${encounter.patients?.last_name||""}`.trim())}</strong> · ${safe(encounter.patients?.snau||"UNIN pending")}</div><div id="encounter-workflow"></div>`);
    const host=modal.querySelector("#encounter-workflow");
    const saveWorkflow=async patch=>{const row={...patch,encounter_id:encounterId,patient_id:encounter.patient_id,updated_by:session.user.id,updated_at:new Date().toISOString()};const {data,error}=await supabase.from("encounter_clinical_workflows").upsert(row,{onConflict:"encounter_id"}).select().single();if(error)throw error;workflow=data};
    const placeSelectedOrders=async(type,form)=>{const selected=[...form.querySelectorAll('[name="catalog_id"]:checked')].map(input=>catalog.find(item=>String(item.id)===input.value)).filter(Boolean);if(!selected.length)return;const service=services.find(item=>{const text=`${item.departments?.name||""} ${item.name}`.toLowerCase();return type==="lab"?/laborator|patholog/.test(text):/radiolog|imaging|x-ray|xray/.test(text)});if(!service)throw new Error(`No active ${type==="lab"?"laboratory":"imaging"} service is configured for this facility.`);for(const item of selected){const {data:order,error}=await supabase.from("orders").insert({patient_id:encounter.patient_id,encounter_id:encounterId,ordering_provider_id:currentProviderId,order_type:type,service_id:service.id,status:"pending"}).select("id").single();if(error)throw error;const detail=type==="lab"?{order_id:order.id,test_code:item.code||item.name,priority:"Routine"}:{order_id:order.id,modality:item.code||item.name,contrast:false};const {error:detailError}=await supabase.from(`${type}_orders`).insert(detail);if(detailError)throw detailError}};
    const render=async()=>{
      const index=Math.max(0,workflowSteps.findIndex(step=>step[0]===workflow.current_step)),[key,title,help]=workflowSteps[index];
      if(["laboratory","imaging"].includes(key)){
        const choices=catalog.filter(item=>item.order_type===(key==="laboratory"?"lab":"imaging"));host.innerHTML=`<p class="workflow-progress">Step ${index+1} of ${workflowSteps.length}</p><h2>${safe(title)}</h2><p>${safe(help)}</p><form id="workflow-form"><label>Search examinations<input type="search" data-exam-search placeholder="Search by name or code"></label><div class="exam-checklist">${choices.map(item=>`<label data-exam-item><input type="checkbox" name="catalog_id" value="${item.id}"><span><strong>${safe(item.name)}</strong><small>${safe(item.code||"No local code")} · ${safe(item.use_text||"")}</small></span></label>`).join("")}</div><footer><button type="button" class="button secondary" data-close>Close</button><button class="button primary">Save and next</button></footer></form>`;
        const form=host.querySelector("#workflow-form"),search=form.querySelector("[data-exam-search]");search.addEventListener("input",()=>form.querySelectorAll("[data-exam-item]").forEach(item=>item.hidden=!item.textContent.toLowerCase().includes(search.value.toLowerCase())));form.querySelector("[data-close]").addEventListener("click",closeModal);form.addEventListener("submit",async event=>{event.preventDefault();try{await placeSelectedOrders(key==="laboratory"?"lab":"imaging",form);await saveWorkflow({current_step:workflowSteps[index+1][0]});notify("Selections saved to the encounter.");await render()}catch(error){notify(error.message)}});return;
      }
      if(key==="diagnosis"){
        if(!await verifiedOnline()){host.innerHTML=`<p class="workflow-progress">Step ${index+1} of ${workflowSteps.length}</p><h2>Suggested Diagnosis</h2><div class="alert">Suggestions are available only while the system is online. Connectivity could not be verified. No diagnosis was generated.</div><footer><button type="button" class="button secondary" data-close>Close</button></footer>`;host.querySelector("[data-close]").onclick=closeModal;return}
        const [releaseResult,suggestionResult,evidenceResult]=await Promise.all([supabase.from("terminology_releases").select("id,system,version").eq("active",true).eq("system","ICD-11 MMS").maybeSingle(),supabase.from("diagnostic_suggestion_rules").select("id,rule_name,source_url,evidence_requirements,terminology_concepts(code,display)").eq("active",true).or(currentFacilityId?`facility_id.is.null,facility_id.eq.${currentFacilityId}`:"facility_id.is.null").limit(100),supabase.from("orders").select("order_type,lab_orders(test_code,lab_results(analyte_code,interpretation,validation_status)),imaging_orders(modality,imaging_results(interpretation,result_status))").eq("encounter_id",encounterId).in("order_type",["lab","imaging"])]);const release=releaseResult.data,evidence=evidenceResult.data||[];const facts={lab:evidence.flatMap(order=>(order.lab_orders||[]).flatMap(test=>(test.lab_results||[]).filter(result=>result.validation_status==="validated").map(result=>({code:result.analyte_code||test.test_code,interpretation:result.interpretation})))),imaging:evidence.flatMap(order=>(order.imaging_orders||[]).flatMap(test=>(test.imaging_results||[]).filter(result=>result.result_status==="final").map(result=>({code:test.modality,interpretation:result.interpretation}))))};const requirementMatches=requirement=>facts[requirement.type]?.some(fact=>String(fact.code).toLowerCase()===String(requirement.code).toLowerCase()&&(!requirement.interpretations?.length||requirement.interpretations.includes(fact.interpretation)));const suggestions=(suggestionResult.data||[]).map(rule=>{const requirements=Array.isArray(rule.evidence_requirements?.all)?rule.evidence_requirements.all:[],matched=requirements.filter(requirementMatches).length;return {...rule,matched,total:requirements.length,probability:requirements.length?Math.round(matched*100/requirements.length):0}}).filter(rule=>rule.matched>0).sort((a,b)=>b.probability-a.probability);
        host.innerHTML=`<p class="workflow-progress">Step ${index+1} of ${workflowSteps.length}</p><h2>Suggested Diagnosis</h2><div class="live-banner"><strong>Information gathered:</strong> SOAP history and symptoms, physical examination, presumptive diagnosis, validated laboratory results, and final imaging reports available in this encounter.</div><p class="alert">Decision support only. Percentages represent matched configured evidence criteria, not disease certainty. A qualified clinician must review, order further tests when appropriate, rule out alternatives, and select the certified diagnosis.</p><div class="suggested-diagnoses">${suggestions.length?suggestions.map(item=>`<div><strong>${safe(item.terminology_concepts?.code)} · ${safe(item.terminology_concepts?.display)}</strong><small>${safe(item.probability)}% rule match · ${safe(item.matched)}/${safe(item.total)} evidence criteria · ${safe(item.rule_name)} · <a href="${safe(item.source_url)}" target="_blank" rel="noopener">Evidence source</a></small></div>`).join(""):'<p class="empty">No reviewed decision-support rule is configured or matched. The system will not invent a diagnosis.</p>'}</div><h2>Certified Diagnosis <small>(Definitive Diagnosis)</small></h2>${release?`<p>Installed terminology: ${safe(release.system)} · ${safe(release.version)}</p><form id="workflow-form"><label>Search ICD-11 code or diagnosis<input type="search" data-icd-search required autocomplete="off"></label><div class="icd-results" data-icd-results><p class="empty">Enter at least two characters.</p></div><input type="hidden" name="concept_id" required><footer><button type="button" class="button secondary" data-close>Close</button><button class="button primary">Save certified diagnosis and next</button></footer></form>`:'<div class="alert">No exact ICD-11 MMS release is active. A platform administrator must import and activate a WHO release before a definitive coded diagnosis can be saved.</div><footer><button class="button secondary" data-close>Close</button></footer>'}`;
        host.querySelector("[data-close]").addEventListener("click",closeModal);if(!release)return;
        const form=host.querySelector("#workflow-form"),search=form.querySelector("[data-icd-search]"),results=form.querySelector("[data-icd-results]");let timer;search.addEventListener("input",()=>{clearTimeout(timer);timer=setTimeout(async()=>{const term=search.value.trim();if(term.length<2){results.innerHTML='<p class="empty">Enter at least two characters.</p>';return}const {data,error}=await supabase.from("terminology_concepts").select("id,code,display,entity_uri,language_code").eq("terminology_release_id",release.id).eq("active",true).or(`code.ilike.%${term.replaceAll(",","")}%,searchable_text.ilike.%${term.replaceAll(",","")}%`).limit(30);results.innerHTML=error?`<p class="alert">${safe(error.message)}</p>`:(data||[]).map(item=>`<button type="button" data-concept='${safe(JSON.stringify(item))}'><strong>${safe(item.code)}</strong> ${safe(item.display)}</button>`).join("")||'<p class="empty">No code found in the installed release.</p>';results.querySelectorAll("[data-concept]").forEach(button=>button.addEventListener("click",()=>{const concept=JSON.parse(button.dataset.concept);form.elements.concept_id.value=concept.id;search.value=`${concept.code} · ${concept.display}`;form.dataset.concept=JSON.stringify(concept)}))},250)});
        form.addEventListener("submit",async event=>{event.preventDefault();try{if(!form.dataset.concept)throw new Error("Select a diagnosis from the installed ICD-11 results.");const concept=JSON.parse(form.dataset.concept);const {error}=await supabase.from("diagnoses").insert({patient_id:encounter.patient_id,encounter_id:encounterId,terminology_release_id:release.id,code:concept.code,entity_uri:concept.entity_uri,display:concept.display,language_code:concept.language_code,diagnosing_provider_id:currentProviderId,clinical_status:"active",diagnosing_provider_name:document.querySelector(".user-menu strong")?.textContent||null});if(error)throw error;await saveWorkflow({current_step:"prescription"});notify("Certified diagnosis saved from the installed ICD-11 release.");await render()}catch(error){notify(error.message)}});return;
      }
      if(key==="follow_up"){
        host.innerHTML=`<p class="workflow-progress">Step ${index+1} of ${workflowSteps.length}</p><h2>Follow-up appointment</h2><form id="workflow-form"><label>Date and time<input type="datetime-local" name="follow_up_at"></label><label>Follow-up instructions<textarea name="follow_up_notes" rows="6">${safe(workflow.follow_up_notes||"")}</textarea></label><footer><button type="button" class="button secondary" data-close>Close</button><button class="button primary">Save and next</button></footer></form>`;
        const form=host.querySelector("#workflow-form");form.querySelector("[data-close]").onclick=closeModal;form.onsubmit=async event=>{event.preventDefault();try{await saveWorkflow({follow_up_at:form.elements.follow_up_at.value?new Date(form.elements.follow_up_at.value).toISOString():null,follow_up_notes:form.elements.follow_up_notes.value||null,current_step:"conclusion"});await render()}catch(error){notify(error.message)}};return;
      }
      if(key==="conclusion"){
        host.innerHTML=`<p class="workflow-progress">Step ${index+1} of ${workflowSteps.length}</p><h2>Encounter conclusion</h2><form id="workflow-form"><label>Patient status<select name="patient_status" required><option value="outpatient">Outpatient</option><option value="hospitalization">Hospitalization</option><option value="deceased">Death / deceased</option></select></label><label>Action<select name="encounter_action"><option value="">No additional action</option><option value="refer">Refer</option><option value="admit">Admit</option><option value="transfer">Transfer</option></select></label><label>Destination / receiving service<input name="action_destination" value="${safe(workflow.action_destination||"")}"></label><footer><button type="button" class="button secondary" data-close>Close</button><button class="button primary">Complete encounter</button></footer></form>`;
        const form=host.querySelector("#workflow-form");form.elements.patient_status.value=workflow.patient_status||"outpatient";form.elements.encounter_action.value=workflow.encounter_action||"";form.querySelector("[data-close]").onclick=closeModal;form.onsubmit=async event=>{event.preventDefault();try{const values=Object.fromEntries(new FormData(form));await saveWorkflow({patient_status:values.patient_status,encounter_action:values.encounter_action||null,action_destination:values.action_destination||null,current_step:"complete",completed_at:new Date().toISOString()});const status="closed";const result=await supabase.from("encounters").update({status,end_at:new Date().toISOString()}).eq("id",encounterId);if(result.error)throw result.error;closeModal();notify("Encounter completed and conclusion saved.")}catch(error){notify(error.message)}};return;
      }      const multiline=key!=="presumed_diagnosis";host.innerHTML=`<p class="workflow-progress">Step ${index+1} of ${workflowSteps.length}</p><h2>${safe(title)}</h2><p>${safe(help)}</p><form id="workflow-form"><label>${safe(title)}<textarea name="content" rows="${multiline?10:5}" required>${safe(workflow[key]||"")}</textarea></label><footer><button type="button" class="button secondary" data-close>Close</button><button class="button primary">Save and next</button></footer></form>`;const form=host.querySelector("#workflow-form");form.querySelector("[data-close]").addEventListener("click",closeModal);form.addEventListener("submit",async event=>{event.preventDefault();try{await saveWorkflow({[key]:form.elements.content.value,current_step:workflowSteps[index+1][0]});notify(`${title} saved.`);await render()}catch(error){notify(error.message)}})
    };await render();
  }catch(error){notify(error.message)}
}

const interpretationLabel=value=>value==="dangerously_abnormal"?"Dangerously abnormal":value==="out_of_range"?"Out of range":value==="unconfigured"?"Threshold not configured":"Normal";
...
async function setSignedInUser() {
  ...
  const {data:facility}=currentFacilityId?await supabase.from("facilities").select("name").eq("id",currentFacilityId).maybeSingle():{data:null};
  const name = profile?.display_name || session.user.email;
  const roleName = role?.roles?.role_name || "Pending role";
  const menu = document.querySelector(".user-menu");
  menu.innerHTML = `<span class="avatar">${name.split(/\s+/).map(x=>x[0]).slice(0,2).join("").toUpperCase()}</span><span><strong>${name}</strong><small>${safe(facility?.name||roleName)} · ${roleName}</small></span><button type="button" class="switch-facility" data-switch-facility>Switch</button><span>⌄</span>`;
  menu.addEventListener("click",async event=>{if(event.target.closest("[data-switch-facility]")){event.stopPropagation();sessionStorage.removeItem(ACTIVE_FACILITY_KEY);location.reload();return}await supabase.auth.signOut();location.reload()});
...
document.addEventListener("click", event => {
  ...
  const vitalAction=event.target.closest("[data-patient-vitals]");
  if(vitalAction){event.preventDefault();event.stopImmediatePropagation();openVitalTrends(Number(vitalAction.dataset.patientVitals));return}
  const deletedAction=event.target.closest("[data-patient-deleted]");
  if(deletedAction){event.preventDefault();event.stopImmediatePropagation();openDeletedRecordHistory(Number(deletedAction.dataset.patientDeleted));return}
...
if (!session) authGate(); else if(session.user.app_metadata?.must_change_password===true) passwordChangeGate(); else if(await requireMfa(supabase,document.body)){ if(await loadClinicalContext()){await setSignedInUser();await hydrateLivePatients();await hydrateDashboard();if(new URLSearchParams(location.search).get("register")==="patient")await patientForm();} }
supabase.auth.onAuthStateChange((event,newSession)=>{
  ...
  if(event==="SIGNED_OUT"){sessionStorage.removeItem(ACTIVE_FACILITY_KEY);location.reload()}
});
