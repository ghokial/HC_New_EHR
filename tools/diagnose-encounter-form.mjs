...
await page.route("**/vendor/supabase.js", route => route.fulfill({
  ...
  body: `globalThis.supabase={createClient(){
    ...
    const chainFor=table=>{let chain;chain=new Proxy({}, {get(_target,property){
      if(property==="maybeSingle"||property==="single")return async()=>({data:table==="providers"?{id:8}:table==="facility_memberships"?{facility_id:9}:null,error:null});
...
const result={departmentNames,initiallyDisabled,initialServiceText,serviceNames,serviceValues,enabledAfterSelection,messages};
console.log(JSON.stringify(result,null,2));
await browser.close().catch(()=>{});
const passed=JSON.stringify(departmentNames)==='["Select department","Anesthesiology","Cardiology"]'
  && initiallyDisabled
  && JSON.stringify(initialServiceText)==='["Select department first"]'
  && JSON.stringify(serviceNames)==='["Select service","Alpha Service","Zulu Service"]'
  && JSON.stringify(serviceValues)==='["","11","12"]'
