const patients = [
  {id:1, initials:"AM", name:"Alain M.", mrn:"HC-100241", unin:"10L-M192-00003", dob:"12 Mar 1992", sex:"Male", type:"Outpatient", service:"Cardiology", provider:"Assigned provider", status:"Open", time:"08:30"},
...
const patientRows = list => list.map(p => `<tr data-patient="${p.id}"><td><div class="patient-cell"><span class="patient-avatar">${p.initials}</span><span><strong>${p.name}</strong><small>${p.mrn}</small></span></div></td><td>${p.dob}</td><td>${p.unin}</td><td>${p.service}</td><td>${p.provider}</td><td>${status(p.status)}</td><td>${p.time}</td></tr>`).join("");
...
function encounters(){return moduleList("Encounter management","Encounters","Visits, admissions, emergency episodes, and observation records.",[["Outpatient · Cardiology","Alain M. · 08:30","Open"],["Inpatient · Medicine","Chantal K. · 09:15","Open"],["Emergency","Joseph M. · 10:00","Closed"]],["Encounter","Patient / time","Status"],"Open encounter")}
...
function registry(){return `${head("Identity interoperability","UNIN registry","Unique National Identity Number / SNAU workflows, family links, and validation.",`<button class="button primary" data-action="enroll">＋ Start enrollment</button>`)}
  <section class="registry-banner"><div><h2>National registry linkage</h2><p>The UNIN/SNAU is the stable join key between the registry, EHR, Healthcarology portals, and authorized sector systems. The country prefix is retained internally and hidden in normal display.</p></div><div class="unin-code">PPCC-SXYYY-CTUUUU</div></section>
